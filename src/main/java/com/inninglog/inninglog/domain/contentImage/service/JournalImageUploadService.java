...
public class JournalImageUploadService {
    ...
    public ImageListUploadResDto getJournalImagePresignedUrlList(List<ImageUploadReqDto> dtos, Long memberId) {
        List<ImageUploadResDto> result = new ArrayList<>();

        for (ImageUploadReqDto dto : dtos) {
            String url = preSginedPutService.journalPutPreUrl(memberId, dto.fileName(), dto.contentType());
            String key = "journal/" + memberId + "/" + dto.fileName();
